def k_largest_elements(root, k):
    ...
    def supporter(node):
        if not node: return
        if len(result) < k and node.right: supporter(node.right)
        if len(result) < k: result.append(node.data)
        if len(result) < k and node.left: supporter(node.left)
