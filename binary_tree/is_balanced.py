...
IsBalancedWithHeight = collections.namedtuple('IsBalancedWithHeight', ('is_balanced', 'height'))

def is_balanced(root):
    if not root:
        return IsBalancedWithHeight(True, -1)
    
    left_balance = is_balanced(root.left)
    if not left_balance.is_balanced:
        return IsBalancedWithHeight(False, 0)
    
    right_balance = is_balanced(root.right)
    if not right_balance.is_balanced:
        return IsBalancedWithHeight(False, 0)
    
    both_balanced = abs(left_balance.height-right_balance.height) <= 1
    height = max(left_balance.height, right_balance.height) + 1
    return IsBalancedWithHeight(both_balanced, height)
...
print(is_balanced(n1).is_balanced)